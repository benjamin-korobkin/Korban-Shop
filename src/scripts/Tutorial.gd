...
#positions that customer nodes iterate trough and see if their open.
...
func spawn_tutorial_gui():
	var tutorialGuiRsc = GameResources.scenesDict["tutorialGui"].instance()
...
#spawns special tutorial customer
func spawn_tutorial_customer():
	yield(get_tree(),"idle_frame")
	var customerRsc = GameResources.scenesDict["tutorialCustomer"].instance()
	customerRsc.position = rect_position
	connect("setOrder",customerRsc,"get_order")
	connect("setPosition",customerRsc,"set_pos")
	emit_signal("setPosition",get_customer_position())
